...
export const handler = async (event: CustomAuthorizerEvent): Promise<CustomAuthorizerResult> => {
  try {
    ...
    logger.info('An User was authorized');
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  const cert: string = await getSignKey(keyId);
...
async function getSignKey(keyId: string): Promise<string> {
  if (auth0Cert) {
    return auth0Cert;
  }
  ...
  const signingKeys: Array<string> = keys
    .filter(
      (key) =>
        key.use === 'sig' && // JWK property `use` determines the JWK is for signature verification
        key.kty === 'RSA' && // We are only supporting RSA (RS256)
        key.kid === keyId && // The `kid` must be present to be useful for later
